...
impl LegacyBitcoinAddress {
    ...
    pub fn from_b58(addrb58: &str) -> Result<LegacyBitcoinAddress, Error> {
        let bytes = base58::from_check(addrb58).map_err(|_e| Error::InvalidByteSequence)?;

        if bytes.len() != 21 {
            m2_test_debug!("Invalid address: {} bytes", bytes.len());
        ...
        let Some((addrtype, network_id)) = legacy_version_byte_to_address_type(*version) else {
            m2_test_debug!("Invalid address: unrecognized version {}", version);
            return Err(Error::InvalidByteSequence);
        };

        let payload_bytes: &[u8; 20] = bytes
            .get(1..21)
            .ok_or_else(|| Error::InvalidByteSequence)?
            .try_into()
...
impl SegwitBitcoinAddress {
    ...
    pub fn from_bech32(s: &str) -> Option<SegwitBitcoinAddress> {
        let (hrp, quintets, variant) = bech32::decode(s)
            .inspect_err(|_e| {
                m2_test_debug!("Failed to decode '{s}': {_e:?}");
            })
            .ok()?;
        ...
        let network_type = match hrp.as_str() {
            ...
            _ => {
                m2_test_debug!("Unrecognized hrp '{:?}'", &hrp);
                return None;
            }
        };

        if quintets.is_empty() || quintets.len() > 65 {
            m2_test_debug!("Invalid prog length: {}", quintets.len());
            return None;
        }
        ...
        let bytes = Vec::from_base32(&prog)
            .inspect_err(|_e| {
                m2_test_debug!("Failed to decode quintets: {_e:?}");
            })
            .ok()?;
...
mod tests {
    ...
    fn test_from_bech32() {
        ...
        for fixture in fixtures.iter() {
            m2_test_debug!("Test '{}'", &fixture.addr);
