...
impl BitcoinOpSigner {
    /// Creates a new `BitcoinOpSigner` from the given private key.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// A new instance of `BitcoinOpSigner`.
    ...
    /// Create a deterministic signer from the seed, with a default nakamoto mining key derived
    ///  from the same seed (
    pub fn default(seed: Vec<u8>) -> BitcoinOpSigner {
        let secret_key = Self::make_secret_key_from_bytes(&seed);
        Self {
            secret_key,
            is_disposed: false
        }
...
/// Test-only utilities for `BitcoinOpSigner`
#[cfg(test)]
impl BitcoinOpSigner {
    ...
    /// Returns a new `BitcoinOpSigner` instance using the same secret key,
...
mod tests {
    ...
    fn test_get_secret_key_as_wif() {
        ...
        let op_signer = BitcoinOpSigner::new(secret);
    ...
    fn test_get_secret_key_as_hex() {
        ...
        let op_signer = BitcoinOpSigner::new(secp_k);
