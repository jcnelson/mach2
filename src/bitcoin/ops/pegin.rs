...
impl OpPegIn {
    ...
    pub fn with_spender(mut self, pubk: &Secp256k1PublicKey) -> Self {
    ...
    fn sign_pegin(
        ...
        let prev_pegin_p2wsh = self.p2wsh_pegin_script_pubkey();
        ...
        for (i, utxo) in utxos_set.utxos.iter().enumerate() {
            ...
            let sig_hash = if !is_cosigner && utxo.script_pub_key == spender_p2wpkh && self.is_spender_signer(signer) {
                // spending on-chain p2pwkh UTXO from spender
            ...
            if !is_cosigner && utxo.script_pub_key == spender_p2wpkh && self.is_spender_signer(signer) {
                // spending on-chain p2wpkh UTXO from spender
                ...
                // spending on-chain p2wpkh UTXO from user
