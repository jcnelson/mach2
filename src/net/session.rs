...
impl NodeSession {
    ...
    fn make_peer_message(&mut self, payload: StacksMessageType) -> Result<StacksMessage, Error> {
    ...
    fn send_peer_message(&mut self, msg: StacksMessage) -> Result<(), Error> {
        Ok(msg.consensus_serialize(&mut self.tcp_socket)?)
    }

    /// Receive a p2p message.
    /// Returns error text on failure.
    fn recv_peer_message(&mut self) -> Result<StacksMessage, Error> {
    ...
    /// Returns error text on failure.
    pub fn begin(data_addr: SocketAddr, replica_peer_addr: SocketAddr) -> Result<Self, Error> {
        // get /v2/info
        let peer_info = run_get_info(&data_addr)?;

        // convert `pox_consensus` and `stable_pox_consensus` into their respective burn block
        // hashes
        let sort_info = run_get_sortition_info(
            &data_addr,
            "consensus",
            &format!("{}", &peer_info.pox_consensus),
        )?
        .pop()
        .ok_or_else(|| Error::SessionError(format!("No sortition returned for {}", &peer_info.pox_consensus)))?;

        let stable_sort_info = run_get_sortition_info(
            &data_addr,
            "consensus",
