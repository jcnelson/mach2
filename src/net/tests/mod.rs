...
fn test_decode_http_request_err() {
    let tests = vec![
        ("GET /foo HTTP/1.1\r\n", Error::DeserializeError("".to_string())),
        ("GET /foo HTTP/\r\n\r\n", Error::DeserializeError("".to_string())),
        (
            "GET /foo HTTP/1.1\r\nHost:",
            Error::DeserializeError("".to_string()),
    ...
    for (data, expected_err_type) in tests.iter() {
        ...
        match (err, expected_err_type) {
            (Error::DeserializeError(..), Error::DeserializeError(..)) => {}
            (Error::MalformedRequest(..), Error::MalformedRequest(..)) => {}
            (x, y) => {
                m2_error!("expected error mismatch: {:?} != {:?}", &y, &x);
                panic!();
            }
        }
    }
...
fn test_decode_http_response_err() {
    ...
    for (data, expected_err_type) in tests.iter() {
        ...
        match (err_type, expected_err_type) {
            ...
            (Error::DeserializeError(_), Error::DeserializeError(_)) => {}
