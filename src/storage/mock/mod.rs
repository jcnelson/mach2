...
impl StackerDBClient for MockStackerDBClient {
    ...
    fn list_chunks(&mut self) -> Result<Vec<SlotMetadata>, Error> {
        if let Some(error) = self.mock_failure.take() {
            return Err(error);
        }
        ...
        m2_test_debug!("list_chunks: {:?}", &metadata);
        Ok(metadata)
    }
    ...
    fn get_chunks(
        ...
        m2_test_debug!("get_chunks({:?}): {:?}", slots_and_versions, &ret);
        Ok(ret)
    }
    ...
    fn get_latest_chunks(
        ...
        m2_test_debug!("get_latest_chunks({:?}): {:?}", slot_ids, &ret);
        Ok(ret)
    }
    ...
    fn put_chunk(
        ...
        if let Some(bad_ack) = self.mock_ack_failure.take() {
            return Ok(bad_ack);
        }
        ...
        m2_test_debug!("put_chunk({:?}): {:?}", &chunk, &ret);
        Ok(ret)
    }
...
impl LocalStackerDBClient {
    pub fn open(path: &str) -> Result<Self, Error> {
        if path != ":memory:" && !std::fs::metadata(path).is_ok() {
            return Err(Error::StorageError("Mock StackerDB database file already exists".into()));
        }
    ...
    fn inner_get_config(conn: &Connection) -> Result<LocalStackerDBConfig, Error> {
        match query_row(conn, "SELECT * FROM config", rusqlite::params![]) {
            ...
            Ok(None) => Err(Error::StorageError("no such row".into())),
            Err(e) => Err(e.into()),
        }
    }
