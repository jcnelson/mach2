...
use crate::vm::special::handle_m2_contract_call_special_cases;
...
impl M2DB {
    fn setup_db(path_str: &str, contract_id: &QualifiedContractIdentifier) -> Result<(Connection, bool), Error> {
        let mut path = PathBuf::from(path_str);
        path.push(contract_id.to_string());
        ...
        SqliteConnection::initialize_conn(&tx).map_err(|e| {
            m2_error!("Failed to initialize DB: {:?}", &e);
            Error::InitializationFailure
        })?;

        m2_debug!("Instantiate M2DB for {} at {}", contract_id, path_str);
    ...
    fn open_readonly(path_str: &str, contract_id: &QualifiedContractIdentifier) -> Result<Connection, Error> {
        let mut path = PathBuf::from(path_str);
        path.push(contract_id.to_string());
        path.push("db.sqlite");
    ...
    pub fn open(
        ...
        let (conn, created) = M2DB::setup_db(path_str, contract_id)?;
    ...
    pub fn headers_db(&self) -> M2HeadersDB {
        let conn = Self::open_readonly(&self.db_path, &self.contract_id)
    ...
    pub fn begin_read_only<'a>(&'a self, at_block: Option<&StacksBlockId>) -> ReadOnlyM2Store<'a> {
...
impl WriteBuffer {
    ...
    pub fn dump(
        ...
        let next_height = match get_m2_block_height(conn, cur_tip) {
...
impl<'a> ClarityBackingStore for ReadOnlyM2Store<'a> {
    fn get_side_store(&mut self) -> &Connection {
        &self.conn
    }

    fn get_cc_special_cases_handler(&self) -> Option<SpecialCaseHandler> {
        Some(&handle_m2_contract_call_special_cases)
    }
    ...
    fn get_data(&mut self, key: &str) -> Result<Option<String>, clarity_error> {
        m2_test_debug!("Get hash for '{}' at height {}", key, self.tip_height);
    ...
    fn put_all_data(&mut self, _items: Vec<(String, String)>) -> Result<(), clarity_error> {
        m2_error!("Attempted to commit changes to read-only K/V");
    ...
    fn insert_metadata(
        ...
        m2_error!("Attempted to write metadata to read-only K/V");
...
impl<'a> WritableM2Store<'a> {
    ...
    pub fn commit_to(mut self, final_bhh: &StacksBlockId) -> Result<(), Error> {
        ...
        m2_test_debug!("commit_to({} --> {})", target_tip, final_bhh);
...
impl<'a> ClarityBackingStore for WritableM2Store<'a> {
    fn set_block_hash(&mut self, bhh: StacksBlockId) -> Result<StacksBlockId, ClarityError> {
