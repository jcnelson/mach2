...
/// Get the next chain tip for the Mach2 headers DB
pub fn make_m2_chain_tip(cur_height: u64) -> StacksBlockId {
    let mut bytes = [0u8; 32];
    bytes[0..8].copy_from_slice(&cur_height.to_be_bytes());
    StacksBlockId(bytes)
}

/// Get the chain tip of the Mach2 headers DB
pub fn get_m2_chain_tip(conn: &Connection) -> StacksBlockId {
    let mut stmt = conn
...
/// Get the block height of the Mach2 headers DB
pub fn get_m2_block_height(conn: &Connection, block_id: &StacksBlockId) -> Option<u64> {
    let mut stmt = conn
